...
class CastTests extends MovieApiApplicationTests {
	...
	void createCast() {
		...
		URI new_cast_location = authRT().postForLocation("/casts", cast, Void.class);
		assertNotNull(new_cast_location);
		ResponseEntity<Cast> response = authRT().getForEntity(new_cast_location, Cast.class);
	...
	@DirtiesContext
	void deleteCast() {
		ResponseEntity<Cast> response = authRT().getForEntity("/casts/1", Cast.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
		ResponseEntity<Void> deleteResponse = authRT().exchange("/casts/1", HttpMethod.DELETE, null, Void.class);
		assertThat(deleteResponse.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);
		response = authRT().getForEntity("/casts/1", Cast.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
		Movie[] movies = authRT().exchange(
		...
		Series[] series = authRT().getForObject("/series", Series[].class);
	...
	@Test
	void getCastList() {
		ResponseEntity<Cast[]> response = authRT().getForEntity("/casts", Cast[].class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
	...
	void updateCast() {
		...
		authRT().put("/casts/1", cast);
		Cast updatedCast = authRT().getForObject("/casts/1", Cast.class);
	...
	void updatedCastNotFound() {
		...
		authRT().put("/casts/10", cast);
		ResponseEntity<Cast> response = authRT().getForEntity("/casts/10", Cast.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
