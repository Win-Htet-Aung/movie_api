...
public class EpisodeTests extends MovieApiApplicationTests {
    ...
    public void getEpisodeList() {
        ResponseEntity<Episode[]> response = authRT().getForEntity("/episodes", Episode[].class);
    ...
    public void createEpisode() throws ParseException {
        ...
        URI new_episode_location = authRT().postForLocation("/episodes?season_id=1", episode);
        ResponseEntity<Episode> response = authRT().getForEntity(new_episode_location, Episode.class);
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
    ...
    public void createEpisodeNoSeason() throws ParseException {
        Episode episode = new Episode(1, "Episode 1 of House of Cards Season 1.", df.parse("2013-11-07"), 8.8);
        ResponseEntity<Void> response = authRT().postForEntity("/episodes", episode, Void.class);
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        response = authRT().postForEntity("/episodes?season_id=10", episode, Void.class);
    ...
    public void updateEpisode() throws ParseException {
        ...
        authRT().put("/episodes/1", episode);
        ResponseEntity<Episode> response = authRT().getForEntity("/episodes/1", Episode.class);
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody().getAir_date()).isEqualTo(episode.getAir_date());
        assertThat(response.getBody().getEpisode_number()).isEqualTo(episode.getEpisode_number());
        assertThat(response.getBody().getTitle()).isEqualTo(episode.getTitle());
        ...
        ResponseEntity<Void> err_response = authRT().exchange("/episodes/1", HttpMethod.PUT, new HttpEntity<>(episode), Void.class);
    ...
    public void updateEpisodeWithRlationships() throws ParseException {
        ...
        authRT().put("/episodes/2", episode);
        season = authRT().getForObject("/seasons/1", Season.class);
        episode = authRT().getForObject("/episodes/2", Episode.class);
        assertThat(season.getEpisodes().size()).isEqualTo(1);
    ...
    public void deleteEpisode() {
        authRT().delete("/episodes/1");
        ResponseEntity<Episode> response = authRT().getForEntity("/episodes/1", Episode.class);
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        Season season = authRT().getForObject("/seasons/2", Season.class);
        assertThat(season.getEpisodes().size()).isEqualTo(1);
    }
}
